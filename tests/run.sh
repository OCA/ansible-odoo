...
$CMD tests/test_$ODOO_INSTALL_TYPE.yml --connection=local --sudo -e "odoo_version=$ODOO_VERSION" || exit 1
...
$CMD tests/test_${ODOO_INSTALL_TYPE}.yml --connection=local --sudo -e "odoo_version=$ODOO_VERSION" -v > $output_log || exit 1
...
$CMD tests/test_${ODOO_INSTALL_TYPE}_changed.yml --connection=local --sudo -e "odoo_version=$ODOO_VERSION" -v > $output_log || exit 1
