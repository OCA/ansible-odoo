...
ansible-playbook -i tests/inventory tests/test_$ODOO_INSTALL_TYPE.yml --connection=local --sudo -e "odoo_version=$ODOO_VERSION pipelining=True" || exit 1
sudo service odoo-$ODOO_INSTALL_TYPE status || exit 1
# Run the role/playbook again, checking to make sure it's idempotent.
output_log=$ODOO_VERSION_$ODOO_INSTALL_TYPE.log
ansible-playbook -i tests/inventory tests/test_${ODOO_INSTALL_TYPE}.yml --connection=local --sudo -e "odoo_version=$ODOO_VERSION pipelining=True" -v > $output_log || exit 1
...
ansible-playbook -i tests/inventory tests/test_${ODOO_INSTALL_TYPE}_changed.yml --connection=local --sudo -e "odoo_version=$ODOO_VERSION pipelining=True" -v > $output_log || exit 1
